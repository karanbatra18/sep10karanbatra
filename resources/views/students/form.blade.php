    ...
    @if(!empty($student) && !empty($student->subjects))
        @foreach($student->subjects as $sub)
            <div class="subject_group">
                <input type="text" class="form-control input_style" name="subjects[name][]" value="{{ $sub->name  }}"
                       placeholder="Subject Name">
                <input type="text" class="form-control input_style" name="subjects[marks][]" value="{{ $sub->marks  }}"
                       placeholder="Marks">
                <input type="text" class="form-control input_style" name="subjects[grade][]" value="{{ $sub->grade  }}"
                       placeholder="Grade">

                <a href="javascript:;" class="remove_subject">Delete This Subject</a>
            </div>
        @endforeach
    @endif
...
<div class="mb-3">
    <a href="javascript:;" class="add_more">Add New Subject</a>
</div>
<div class="mb-3">
